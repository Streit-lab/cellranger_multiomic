class WorkflowScmultiomic {
    ...
    public static void initialise(params, log, valid_params) {
        ...
        if (!params.reference_name) {
            log.error "No name was provided for the reference genome. Please specify with '--reference_name REFERENCE'"
            System.exit(1)
        }
        ...
        if (!valid_params['data_type'].contains(params.data_type)) {
            log.error "Invalid option: '${params.data_type}'. Valid options for '--data_type': ${valid_params['data_type'].join(', ')}."
            System.exit(1)
        }
    }
